...
def always_green(streets):
    ...
    return [
        (intersection, incoming_streets[intersection][0])
        for intersection, count in counter.items()
        if count == 1
    ]
...
def get_slow_cars(cars, streets, duration):
    ...
    useless_cars = [
        i
        for (i, c_duration) in enumerate(car_durations)
        if c_duration[1] > 1.0 * duration
    ]
...
def solve_all():
    ...
    problems = {A: solve, B: solve, C: solve, D: solve, E: solve, F: solve}
    ...
    if current_file == "e.txt":
        ...
        for i, car in enumerate(cars):
            if i not in slow_cars:
                cars2.append(car)
        print(
            "Cars before %d, cars after %d (remove slow cars)" % (len(cars), len(cars2))
        )
        cars = cars2

    n_slow_cars, slow_cars, car_durations = get_slow_cars(
        cars, streets, duration
    )  # refresh
    ...
    for intersection_id in not_assigned:
        ...
        if current_file == "d.txt":
            ...
            RES.append(
                [
                    intersection_id,
                    get_street_weights_distribution(street_weights, in_streets),
                ]
            )
